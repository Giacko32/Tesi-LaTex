...
Ad esempio, se $\mathcal{D} = 6$, il numero totale di nodi è pari a $19.531$. Considerando che per ogni nodo è necessario calcolare i gradienti per tutti i parametri attivi del modello al fine di stimare l'importanza, il tempo necessario per una ricerca esaustiva risulterebbe proibitivo. Tale criticità viene risolta attraverso la fase di \textbf{Bound}, che permette di effettuare un \textit{pruning on-the-fly} sull'albero di ricerca, escludendo interi rami computazionalmente onerosi ma poco promettenti dal punto di vista della funzione obiettivo. \newline
...
Per ogni nodo dell'albero di ricerca, l'applicazione delle azioni di \textit{pruning} genera i cinque nodi figli corrispondenti. Ciascuna azione identifica un gruppo specifico di parametri $\mathcal{W}_g$ all'interno dell'architettura che può essere rimosso minimizzando il degrado delle prestazioni del modello. \newline
...
\textbf{Voluto:} come cambia la loss al variare di $w$.
...
Perturbazione del parametro $w$:
$$ \delta w = -w \quad \text{(annullamento di quel parametro)} $$
$$ L(w + \delta w) - L(w) \approx (-w) \cdot \frac{\partial L}{\partial w} $$

Applicato ad un gruppo di parametri $W_G$:
$$ \Delta L(W_G) = \sum_{w \in W_G} \Delta L(w) = \sum_{w} -w \cdot \frac{\partial L}{\partial w} = - \sum_{w} w \cdot \frac{\partial L}{\partial w} $$

Ma non possiamo fare in questo modo perché la loro somma è $\Delta L(W_G)$, ma in questo modo non è detto che sia corretto. Un'approssimazione della \textbf{SOLUZIONE}: \\
Un approccio più conservativo:
$$ \hat{I}(W_G) = \sum_{w \in W_G} \Delta L(w)^2 \approx \sum_{w} \left( w \cdot \frac{\partial L}{\partial w} \right)^2 $$

Ma come calcolo $\frac{\partial L}{\partial w}$? Non conviene, perché supponiamo $\Delta L(w_1) = - \Delta L(w_2)$. \\
Sui batch? Accumuliamo i gradienti? \\
Sul batch 1: $\frac{\partial L}{\partial w}(b_1) = +1$, sul batch 2: $\frac{\partial L}{\partial w}(b_2) = -1$.
$$ \frac{\partial L}{\partial w}_{tot} = +1 - 1 = 0 \implies I(w) = 0 $$
Scorretto, il parametro è sensibile.

\section*{SOLUZIONE}
$$ I(w) = w^2 \cdot E[g^2] = w^2 \cdot \frac{1}{N} \sum_{i=1}^{N} g_i^2 \quad \text{(medie sui batch)} $$

\section*{GIUSTIFICAZIONE TEORICA}
Di fatto sto usando la Fisher Information Matrix approssimando la ...
$$ F(w) = E_{x,y} \left[ \nabla L(y, f(x,w)) \cdot \nabla L(y, f(x,w))^T \right] \in \mathbb{R}^{|w| \times |w|} $$
$$ F(w_i, w_j) = E \left[ \frac{\partial L}{\partial w_i} \cdot \frac{\partial L}{\partial w_j} \right] $$

Assumendo che i parametri siano indipendenti per questioni computazionali, abbiamo:
$$ F(w_i, w_j) = 0 \quad \text{ed} \quad F(w_i, w_i) = E \left[ \left( \frac{\partial L}{\partial w_i} \right)^2 \right] $$
Quindi:
$$ F_w = \frac{1}{N} \sum \left( \frac{\partial L}{\partial w} \right)^2 $$
dove i singoli campioni, noi lavoriamo su batch per parallelizzare i calcoli. \\
Utilizzando Fisher Importance sto utilizzando l'informazione dell'Hessiano.

\section*{COLLEGAMENTO CON L'HESSIANO}
Assumiamo un problema di classificazione:
$$ \sum_{y \in Y} p(y|x,w) = 1 $$
Facciamo la derivata rispetto ai parametri:
$$ \nabla_w \sum_{y} p(y|x,w) = 0 \implies \sum_{y} \nabla_w p(y|x,w) = 0 $$
Sappiamo che:
$$ \nabla p = p \frac{\nabla p}{p} = p \nabla \log(p) $$
Allora:
$$ \sum_{y} p(y|x,w) \cdot \nabla_w \log p(y|x,w) = 0 $$
Deriviamo un'altra volta (assumendo $p(y|w,x) = p(y)$ per brevità di notazione):
$$ \nabla_w \sum_{y} p(y|x,w) \cdot \nabla_w \log p(y|x,w) = 0 $$
$$ \sum_{y} \left[ \nabla_w p(y) \cdot \nabla_w \log p(y)^T + p(y) \cdot \nabla_w^2 \log p(y) \right] = 0 $$
$$ \sum_{y} \left[ p(y) \nabla_w \log p(y) \cdot \nabla_w \log p(y)^T + p(y) \cdot \nabla_w^2 \log p(y) \right] = 0 $$
$$ \sum_{y} p(y) \left[ \nabla_w \log p(y) \nabla_w \log p(y)^T + \nabla_w^2 \log p(y) \right] = 0 $$
Da cui otteniamo la relazione tra Fisher e Hessiano.

Se usiamo:
$$ E_y[\text{Fisher}] + E_y[\text{Hessiano}] = 0 $$
$$ E_y \left[ \nabla \log p \nabla \log p^T \right] = - E_y \left[ \nabla^2 \log p \right] $$

Se usiamo come loss la cross-entropy, che è una Negative Log-Likelihood (NLL):
$$ L = -\log p(y_{true}|x,w) $$
Da cui:
$$ \nabla L = -\nabla \log p $$
$$ \nabla^2 L = \nabla(-\nabla \log p) = -\nabla^2 \log p $$
Quindi:
$$ E_y \left[ \nabla L \cdot \nabla L^T \right] = E_y \left[ \nabla^2 L \right] $$
$$ E \left[ \frac{\partial L}{\partial w_i} \cdot \frac{\partial L}{\partial w_j} \right] = E \left[ \frac{\partial^2 L}{\partial w_i \partial w_j} \right] $$
Ovvero: Fisher approssima l'Hessiano.

Nella pratica Fisher approssima l'Hessiano solo in situazioni in cui il modello rappresenta la distribuzione dei dati reali in convergenza. Nel nostro metodo possiamo assumere che sia vero solo subito dopo un fine-tuning, perché estraiamo i pesi di importanza.
