...
\begin{itemize}
    ...
    \item Per ogni blocco $i$: $m^{(i)} = \{m_{\mathrm{head}}, m_{\mathrm{attn}},
...
La metrica di importanza derivata nella sezione precedente risulta corrisponde all'approssimazione diagonale della \textbf{Matrice di Informazione di Fisher}, così definita: 
\begin{equation}
...
\end{equation} \newline
Notare che esiste un profondo legame tra l'\textit{Informazione di Fisher} e l'\textit{Hessiano} della funzione di perdita. Di seguito la dimostrazione formale, assumendo che la funzione di perdita sia la \textit{Negative Log-Likelihood}. \newline
...
\end{equation}
Utilizzando la proprietà del log-gradient, per cui $\nabla p = p \frac{\nabla p}{p} = p \nabla \log(p)$, possiamo riscrivere l'espressione come:
\begin{equation}
    \sum_{y} p(y|x,w) \cdot \nabla_w \log p(y|x,w) = 0
\end{equation}
Per brevità di notazione, assumiamo $p(y|x,w) = p(y)$ e deriviamo una seconda volta rispetto a $w^T$:
\begin{equation}
...
\begin{equation}
    \sum_{y} \left[ p(y) \nabla_w \log p(y) \cdot \nabla_w \log p(y)^T + p(y) \cdot \nabla_w^2 \log p(y) \right] = 0
\end{equation}
Raccogliendo $p(y)$, che rappresenta il peso del valore atteso $\mathbb{E}_y$, arriviamo alla relazione fondamentale:
\begin{equation}
    \sum_{y} p(y) \left[ \nabla_w \log p(y) \nabla_w \log p(y)^T + \nabla_w^2 \log p(y) \right] = 0
\end{equation}
Da cui segue che il valore atteso della Fisher Information e dell'Hessiano sono legati dalla relazione:
\begin{equation}
    \mathbb{E}_y [\text{Fisher}] + \mathbb{E}_y [\text{Hessiano}] = 0 \implies \mathbb{E}_y \left[ \nabla \log p \nabla \log p^T \right] = - \mathbb{E}_y \left[ \nabla^2 \log p \right]
\end{equation}
Nel nostro framework, utilizziamo come funzione di perdita la Cross-Entropy, che corrisponde alla Negative Log-Likelihood (NLL):
\begin{equation}
    \mathcal{L} = -\log p(y_{true}|x,w)
\end{equation}
...
\begin{equation}
    \mathbb{E}_y \left[ \nabla \mathcal{L} \cdot \nabla \mathcal{L}^T \right] = \mathbb{E}_y \left[ \nabla^2 \mathcal{L} \right]\end{equation}
Il che implica, per le singole componenti diagonali:
\begin{equation}
    \mathbb{E} \left[ \left( \frac{\partial \mathcal{L}}{\partial w_i} \right)^2 \right] = \mathbb{E} \left[ \frac{\partial^2 \mathcal{L}}{\partial w_i^2} \right]
\end{equation}
Considerazioni PraticheSebbene la teoria garantisca che la matrice di Fisher approssimi l'Hessiano, è importante sottolineare che tale equivalenza è strettamente valida solo quando il modello rappresenta correttamente la distribuzione dei dati reali, ovvero in condizioni di convergenza.Nel metodo DL-DFBnB proposto, questa assunzione è considerata verificata poiché l'estrazione della metrica di importanza avviene su modelli pre-addestrati o immediatamente dopo una fase di fine-tuning. In tale stato di stabilità, la Fisher Information fornisce una stima affidabile della curvatura della perdita, permettendo di identificare con precisione i parametri la cui rimozione minimizza l'impatto sulle prestazioni del Transformer.
...
Se usiamo: $$ E_y[\text{Fisher}] + E_y[\text{Hessiano}] = 0 $$ $$ E_y \left[
        \nabla \log p \nabla \log p^T \right] = - E_y \left[ \nabla^2 \log p \right] $$

Se usiamo come loss la cross-entropy, che è una Negative Log-Likelihood (NLL):
$$ L = -\log p(y_{true}|x,w) $$ Da cui: $$ \nabla L = -\nabla \log p $$ $$
    \nabla^2 L = \nabla(-\nabla \log p) = -\nabla^2 \log p $$ Quindi: $$ E_y \left[
        \nabla L \cdot \nabla L^T \right] = E_y \left[ \nabla^2 L \right] $$ $$ E
    \left[ \frac{\partial L}{\partial w_i} \cdot \frac{\partial L}{\partial w_j}
        \right] = E \left[ \frac{\partial^2 L}{\partial w_i \partial w_j} \right] $$
Ovvero: Fisher approssima l'Hessiano.

Nella pratica Fisher approssima l'Hessiano solo in situazioni in cui il modello
rappresenta la distribuzione dei dati reali in convergenza. Nel nostro metodo
possiamo assumere che sia vero solo subito dopo un fine-tuning, perché
estraiamo i pesi di importanza.
