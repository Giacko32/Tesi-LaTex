...
\begin{algorithmic}[1]
    \State \textbf{Input:} Modello pre-addestrato $M_0$, Dataset $\mathcal{D}$, Iterazioni $N$, Epoche di Fine Tuning $K$
    \State \textbf{Output:} Modello compresso e ottimizzato $M$
    \Statex
    \State $M_{teacher} \gets M_0$ \Comment{Modello originale per la Distillazione}
    \State $M \gets M_0$
    
    \For{$n \gets 1$ \textbf{to} $N$}
        \State \textbf{1. Fase di Campionamento e Ricerca:}
        \State $\mathcal{D}_{search} \gets \text{campiona\_search\_set}(\mathcal{D}_{train})$ \Comment{Estrazione dinamica del set di ricerca}
        \State $m_{pruning} \gets \text{pruning\_NAS}(M, \mathcal{D}_{search})$ \Comment{NAS per ottenere la maschera di pruning}
        \State $M_{pruned} \gets \text{comprimi\_ViT}(M, m_{pruning})$ \Comment{ricostruzione del modello compresso}
        \State \textbf{2. Fase di Recovery Fine-Tuning:}
